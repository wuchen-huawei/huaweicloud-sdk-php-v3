...
class CreateUserOption implements ModelInterface, ArrayAccess
{
    ...
    * Sets domainId
    *
    * @param string $domainId IAM用户所属的账号ID，获取方式请参见：[获取账号、IAM用户、项目、用户组、委托的名称和ID](https://support.huaweicloud.com/api-iam/iam_17_0002.html)。
    *
    * @return $this
    */
    ...
    * Sets password
    *
    * @param string|null $password IAM用户密码。   - 系统默认密码最小长度为6字符，在6-32字符之间支持用户自定义密码长度。   - 至少包含以下四种字符中的两种： 大写字母、小写字母、数字和特殊字符。   - 不能包含手机号和邮箱。   - 必须满足账户设置中密码策略的要求。
    *
    * @return $this
    */
    ...
    * Sets email
    *
    * @param string|null $email IAM用户邮箱，需符合邮箱格式，长度小于等于255字符。
    *
    * @return $this
    */
    ...
    * Sets xuserId
    *
    * @param string|null $xuserId IAM用户在外部系统中的ID。长度小于等于128字符，须与xuser_type同时存在。 >外部系统指与华为云对接的外部企业管理系统，xaccount_type、xaccount_id、xdomain_type、xdomain_id、xuser_type、xuser_id等参数值，无法在华为云获取，请咨询企业管理员。
    *
    * @return $this
    */
